...
const placeholder_handler_1 = require("../placeholder-handler");
...
async function gatsbyImageResolver(source, args, actions, store) {
    ...
    if (!args.placeholder) {
        args.placeholder = placeholder_handler_1.PlaceholderType.DOMINANT_COLOR;
    }
    else if (args.placeholder === placeholder_handler_1.PlaceholderType.TRACED_SVG) {
    ...
    const formats = (0, utils_1.validateAndNormalizeFormats)(args.formats, sourceMetadata.format);
...
function calculateImageSizes(sourceMetadata, { width, height, layout, fit, outputPixelDensities, breakpoints, aspectRatio, }) {
...
function calculateResponsiveImageSizes({ sourceMetadata, width, height, fit = `cover`, outputPixelDensities, breakpoints, layout, aspectRatio: requestedAspectRatio, }) {
    let sizes = [];
    let aspectRatio;
    if (requestedAspectRatio) {
        aspectRatio = requestedAspectRatio;
