...
const extensions_1 = require("../../schema/extensions");
exports.actions = {
    /**
     ...
     */
    addThirdPartySchema: ({ schema }, plugin, traceId) => {
        return {
    ...
    createTypes: (types, plugin, traceId) => {
        return {
            ...
            payload: Array.isArray(types)
                ? types.map(type_defs_1.parseTypeDef)
                : (0, type_defs_1.parseTypeDef)(types),
        };
    },
    /**
    ...
    createFieldExtension: (extension, plugin, traceId) => (dispatch, getState) => {
        ...
        if (!name) {
            reporter_1.default.error(`The provided field extension must have a \`name\` property.`);
        }
        else if (extensions_1.reservedExtensionNames.includes(name)) {
            reporter_1.default.error(`The field extension name \`${name}\` is reserved for internal use.`);
        }
        else if (fieldExtensions[name]) {
            reporter_1.default.error(`A field extension with the name \`${name}\` has already been registered.`);
        }
        else {
            dispatch({
     ...
     * }
     */
    printTypeDefinitions: ({ path = `schema.gql`, include, exclude, withFieldTypes = true, }, plugin, traceId) => {
        return {
    ...
    createResolverContext: (context, plugin, traceId) => (dispatch) => {
        if (!context || typeof context !== `object`) {
            reporter_1.default.error(`Expected context value passed to \`createResolverContext\` to be an object. Received "${context}".`);
        }
        else {
            const { name } = plugin || {};
            const payload = !name || name === `default-site-plugin`
                ? context
                : { [(0, lodash_camelcase_1.default)(name.replace(/^gatsby-/, ``))]: context };
            dispatch({
...
const withDeprecationWarning = (actionName, action, api, allowedIn) => (...args) => {
    reporter_1.default.warn(`Calling \`${actionName}\` in the \`${api}\` API is deprecated. ` +
...
() => {
    reporter_1.default.error(`\`${actionName}\` is not available in the \`${api}\` API. ` +
        `Please use: ${allowedIn.map((a) => `\`${a}\``).join(`, `)}.`);
};
...
const mapAvailableActionsToAPIs = (restrictions) => {
    ...
    actionNames.forEach((actionName) => {
        const action = exports.actions[actionName];
...
exports.availableActionsByAPI = mapAvailableActionsToAPIs({
    createFieldExtension: {
