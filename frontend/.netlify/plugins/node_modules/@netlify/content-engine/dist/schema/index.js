...
const getAllTypeDefinitions = () => {
    const { schemaCustomization: { types }, } = store.getState();
...
const getAllFieldExtensions = () => {
    const { schemaCustomization: { fieldExtensions: customFieldExtensions }, } = store.getState();
    return {
...
const buildInferenceMetadata = ({ types }) => new Promise((resolve) => {
...
const build = async ({ parentSpan, fullMetadataBuild = true }) => {
    ...
    const { schemaCustomization: { thirdPartySchemas, printConfig }, inferenceMetadata, config: { mapping: typeMapping }, } = store.getState();
    const typeConflictReporter = new TypeConflictReporter();
    const fieldExtensions = getAllFieldExtensions();
    const schemaComposer = createSchemaComposer({ fieldExtensions });
    const schema = await (0, schema_1.buildSchema)({
        schemaComposer,
