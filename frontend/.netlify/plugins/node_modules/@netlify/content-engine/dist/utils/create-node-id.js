...
function createNodeId(id, namespace) {
    if (typeof id === `number`) {
        ...
        reporter_1.default.panic(`The \`id\` parameter passed to createNodeId must be a String or Number (got ${typeof id})`);
    }
    else if (typeof namespace !== `string`) {
        reporter_1.default.panic(`The \`namespace\` parameter passed to createNodeId must be a String (got ${typeof namespace})`);
    }
    let nsHash = unprefixedCache.get(namespace);
    if (!nsHash) {
        nsHash = (0, uuid_1.v5)(namespace, seedConstant);
