...
const absoluteOrDirectory = (directory, filePath) => {
    ...
    return path_1.default.join(directory, filePath);
};
const getWindowsEncryptionPassword = async () => {
    ...
    const results = await (0, prompts_1.default)({
        type: `password`,
        name: `value`,
...
async function getSslCert({ name, certFile, keyFile, caFile, directory, }) {
    ...
    reporter_1.default.info(`setting up automatic SSL certificate (may require elevated permissions/sudo)\n`);
    try {
        if ([`linux`, `darwin`].includes(os_1.default.platform()) && !process.env.HOME) {
            ...
            const mkdtemp = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), `home-`));
