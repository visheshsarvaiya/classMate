...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  // === AI Suggestion Function ===
  const fetchAISuggestions = async () => {
    const apiKey = process.env.REACT_APP_GEMINI_API_KEY; // ✅ Correct variable name
    ...
    try {
      ...
      // ✅ Corrected endpoint: gemini-2.0-flash is the latest working model
      const response = await axios.post(
      ...
      toast({
        title: "AI Error",
        description:
          "Failed to fetch AI suggestions. Check your API key or model name.",
        status: "error",
        duration: 5000,
        isClosable: true,
        position: "bottom",
